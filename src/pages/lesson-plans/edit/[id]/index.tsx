...
import { useRoqClient, useLessonPlanFindFirst } from 'lib/roq';
...
function LessonPlanEditPage() {
  ...
  const { data, error, isLoading, mutate } = useLessonPlanFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: LessonPlanInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.lesson_plan.update({
        data: values as RoqTypes.lesson_plan,
        where: {
          id,
        },
      });
