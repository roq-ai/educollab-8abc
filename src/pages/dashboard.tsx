...
import {
  ...
  useFreelanceEducatorCount,
  useLessonPlanCount,
  useEducationalContentCount,
} from 'lib/roq';

import { LearningModuleListPage as ShowCaseList } from 'pages/learning-modules';

function DashboardPage() {
  const { session } = useSession();

  const freelanceEducatorStat = useFreelanceEducatorCount();

  const lessonPlanStat = useLessonPlanCount();

  const educationalContentStat = useEducationalContentCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Freelance Educators',
      result: freelanceEducatorStat?.data,
      isLoading: freelanceEducatorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Lesson Plans',
      result: lessonPlanStat?.data,
      isLoading: lessonPlanStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Educational Contents',
      result: educationalContentStat?.data,
      isLoading: educationalContentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`learning-modules`} passHref legacyBehavior>
                  <Button
