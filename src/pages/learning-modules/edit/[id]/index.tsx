...
import { useRoqClient, useLearningModuleFindFirst } from 'lib/roq';
...
function LearningModuleEditPage() {
  ...
  const { data, error, isLoading, mutate } = useLearningModuleFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: LearningModuleInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.learning_module.update({
        data: values as RoqTypes.learning_module,
        where: {
          id,
        },
      });
